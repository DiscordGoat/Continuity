...
import goat.minecraft.minecraftnew.other.realms.utils.TerrainUtils;
...
public class Tropic implements CommandExecutor, TabCompleter {
    ...
    private final TerrainUtils terrainUtils;

    // Chunk tracking
    private final Set<TerrainUtils.ChunkCoord> landChunks = new HashSet<>();
    private final List<TerrainUtils.ChunkCoord> chunkCoords = new ArrayList<>();
        ...
        if (cmd.getName().equalsIgnoreCase("tropic")) {
            ...
            if (args.length > 0) {
                ...
                if (mode.equals("preview")) {
                    ...
                    player.sendMessage("§cUnknown mode. Use '/tropic preview' or '/tropic performance'");
                    return true;
                }
            } else {
                // Default to preview mode
                currentMode = GenerationMode.PREVIEW;
                player.sendMessage("§eUsing default preview mode. Use '/tropic performance' for fast generation.");
            }
    ...
    private void prepareChunks() {
        ...
        for (int cx = -r; cx <= r; cx++)
            for (int cz = -r; cz <= r; cz++)
                chunkCoords.add(new TerrainUtils.ChunkCoord(cx, cz));
    }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < chunksPerTick && chunkIndex < chunkCoords.size(); i++) {
                    TerrainUtils.ChunkCoord cc = chunkCoords.get(chunkIndex);

                    switch (currentPhase) {
                        case 0: // Ocean Floor
                            terrainUtils.genOceanFloor(tropicWorld, cc.x, cc.z);
                            break;
                        case 1: // Island Base
                            terrainUtils.genIslandBase(tropicWorld, cc.x, cc.z);
                            landChunks.add(cc);
                            break;
                        case 2: // Beaches
                            if (landChunks.contains(cc)) {
                                terrainUtils.paintSuperiorBeach(tropicWorld, cc.x, cc.z);
                            }
                            break;
                        case 3: // Hills
                            if (landChunks.contains(cc)) {
                                terrainUtils.paintHills(tropicWorld, cc.x, cc.z);
                            }
                            break;
                        case 4: // Trees and Sea Mines
                            terrainUtils.plantTree(tropicWorld, cc.x, cc.z, null);
                ...
                if (chunkIndex >= chunkCoords.size()) {
                    ...
                    if (currentPhase == 3 && lighthouseLocation == null) {
                        ...
                        lighthouseLocation = findBestStructureLocation(
                                "Lighthouse", 11, 11, Material.SAND,
                                Arrays.asList(Material.GRASS_BLOCK), 7,
                    ...
                    if (currentPhase == 5) {
                        ...
                        if (!barPlaced) {
                            if (barLocation == null) {
                                barLocation = findBestStructureLocation(
                                        "Bar", 14, 14, Material.SAND,
                                        Arrays.asList(Material.GRASS_BLOCK), 10,
                        ...
                        if (barPlaced && lighthousePlaced) {
                            ...
                            player.sendMessage("§a§lTropic world generation complete!");
    ...
    private void startPhase0() {
        ...
        new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                TerrainUtils.ChunkCoord cc = chunkCoords.get(idx++);
                terrainUtils.genOceanFloor(tropicWorld, cc.x, cc.z);
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (idx < chunkCoords.size()) {
                    // 1) Plant trees
                    TerrainUtils.ChunkCoord cc = chunkCoords.get(idx++);
                    terrainUtils.plantTree(tropicWorld, cc.x, cc.z, player);
                    ...
                    if (!lighthousePlaced) {
                        if (lighthouseLocation != null) {
                            ...
                            // Level the ground using TerrainUtils
                            terrainUtils.levelGround(
                                    tropicWorld,
                                    lightLocation.getBlockX() - 5,
                                    lightLocation.getBlockY(),
                            ...
                            lighthouseLocation = findBestStructureLocation(
                                    ...
                                    11, 11,
                                    Material.SAND,
                                    Arrays.asList(Material.GRASS_BLOCK, Material.DIRT),
                    ...
                    if (barPlaced && lighthousePlaced) {
                        ...
                        player.sendMessage("§a§lTropic world generation complete! You can now explore the island!");
                        player.setGameMode(GameMode.SURVIVAL);
                        cancel();
                    }
                }
    ...
    private Location findBestStructureLocation(
        ...
        for (TerrainUtils.ChunkCoord cc : landChunks) {
                ...
                for (int dx = 0; dx <= 16 - Math.min(width/2, 16); dx++) {
                    for (int dz = 0; dz <= 16 - Math.min(length/2, 16); dz++) {
                        LocationAssessment assessment = assessLocation(
                                ox + dx, oz + dz, width, length,
                                preferredMaterial,
                                Arrays.asList(Material.GRASS_BLOCK, Material.DIRT, Material.SAND, Material.GRAVEL),
        ...
        if (candidates.isEmpty()) {
            ...
            if (!landChunks.isEmpty()) {
                TerrainUtils.ChunkCoord cc = landChunks.iterator().next();
                int x = (cc.x << 4) + 8;
                int z = (cc.z << 4) + 8;
                int y = Math.max(SEA_LEVEL + 2, tropicWorld.getHighestBlockYAt(x, z));

                terrainUtils.levelGround(tropicWorld, x - (width+4)/2, y, z - (length+4)/2, width+4, length+4, preferredMaterial);
    ...
    private LocationAssessment assessLocation(
        ...
        for (int dx = 0; dx < width; dx++) {
            for (int dz = 0; dz < length; dz++) {
                ...
                Material topMaterial = tropicWorld.getBlockAt(x, y, z).getType();
                if (topMaterial == Material.WATER) {
                    return result; // Not suitable
                }

                Material baseMaterial = tropicWorld.getBlockAt(x, y, z).getType();
        ...
        for (int xOffset = -minDistanceFromWater; xOffset <= width + minDistanceFromWater; xOffset++) {
            for (int zOffset = -minDistanceFromWater; zOffset <= length + minDistanceFromWater; zOffset++) {
                ...
                for (int dy = -2; dy <= 2; dy++) {
                    int checkY = Math.min(Math.max(targetHeight + dy, 0), tropicWorld.getMaxHeight());
                    if (tropicWorld.getBlockAt(x, checkY, z).getType() == Material.WATER) {
        ...
        if (existingStructures != null) {
            Location centerLocation = new Location(tropicWorld, startX + width/2.0, targetHeight, startZ + length/2.0);
    ...
    private Location findNearestSolidBlock(Location center, int radius) {
        ...
        List<Material> suitableMaterials = Arrays.asList(
                ...
                Material.SAND, Material.SANDSTONE, Material.BLACKSTONE
        );
